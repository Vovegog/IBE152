...
while play_game:
    ...
    while len(dupe_list) > 0 and not lose_game: # This loop can be moved into a function
        ...
        for i in range(len(cards_on_table)):
            print("Index " + str(cards_on_table.index(cards_on_table[i])) + f" - {cards_on_table[i][0]}, {cards_on_table[i][1]}") # Print index-value and name of cards currently on the table
